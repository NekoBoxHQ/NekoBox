...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), ui(new Ui::MainWindow) {
    ...
    connect(ui->toolButton_document, &QToolButton::clicked, this, [=] { QDesktopServices::openUrl(QUrl("https://github.com/lima-droid/NekoBox")); });
    connect(ui->toolButton_ads, &QToolButton::clicked, this, [=] { QDesktopServices::openUrl(QUrl("https://github.com/lima-droid/NekoBox")); });
    connect(ui->toolButton_update, &QToolButton::clicked, this, [=] { runOnNewThread([=] { CheckUpdate(); }); });
    connect(ui->toolButton_url_test, &QToolButton::clicked, this, [=] { speedtest_current_group(1, true); });
    connect(ui->toolButton_tcping, &QToolButton::clicked, this, [=] { speedtest_current_group(0, true); });
    connect(ui->toolButton_ping, &QToolButton::clicked, this, [=] { ping_current(); });
    ...
    connect(ui->menu_ping, &QAction::triggered, this, [=]() { ping_current(); });
...
void MainWindow::on_menu_profile_debug_info_triggered() {
    ...
    auto box = new QMessageBox(QMessageBox::Information, software_name, ents.first()->ToJsonBytes(), QMessageBox::NoButton, this);
    auto btnEdit = box->addButton(QObject::tr("Edit"), QMessageBox::ActionRole);
    auto btnReload = box->addButton(QObject::tr("Reload"), QMessageBox::ActionRole);
    auto btnOk = box->addButton(QObject::tr("OK"), QMessageBox::AcceptRole);
    box->setDefaultButton(btnOk);
    box->exec();
    if (box->clickedButton() == btnEdit) {
        QDesktopServices::openUrl(QUrl::fromLocalFile(QFileInfo(QStringLiteral("profiles/%1.json").arg(ents.first()->id)).absoluteFilePath()));
    } else if (box->clickedButton() == btnReload) {
