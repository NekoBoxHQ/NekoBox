            ...
            runOnNewThread([&] {
                ...
                forever {
                    ...
                    if (!result.error().empty()) {
                        ...
                        MW_show_log(tr("[%1] 测试成功: %2 ms").arg(profile->bean->DisplayTypeAndName(), Int2String(result.ms())));
                    }
...
void MainWindow::ping_current() {
    auto group = NekoGui::profileManager->CurrentGroup();
    if (group == nullptr) return;
    auto profiles = group->Profiles();
    if (profiles.isEmpty()) return;

    // 遍历全组 ping，受限并发（test_concurrent），每个节点结果独立写回
    struct PingState {
    ...
    st->kick = [this, st]() {
        while (st->started < st->profiles.size() && (st->started - st->finished) < st->maxConcurrent) {
            ...
            st->started++;
            auto host = profile->bean->serverAddress;
            if (host.isEmpty()) {
                st->finished++;
                continue;
            }
            ...
            connect(p, QOverload<int, QProcess::ExitStatus>::of(&QProcess::finished), this, [=](int code, QProcess::ExitStatus) {
                ...
                int pingMs = 0;
                if (m.hasMatch()) {
                    pingMs = m.captured(1).toInt();
                } else {
                    // Linux: round-trip min/avg/max = a/b/c ms
                    QRegularExpression re2("=\\s*[\\d.]+/([\\d.]+)/");
                    auto m2 = re2.match(*output);
                    if (m2.hasMatch()) pingMs = (int) m2.captured(1).toDouble();
                }
                if (pingMs > 0) {
                    profile->latency = pingMs;
                    profile->Save();
                    auto pid = profile->id;
...
#else
            p->start("ping", QStringList{"-c", "4", host});
#endif
        }
    };
...
void MainWindow::CheckUpdate() {
    ...
    if (response.release_download_url() == nullptr) {
        runOnUiThread([=] {
            ...
            dlg->setWindowTitle(QObject::tr("Update"));
            auto layout = new QVBoxLayout(dlg);
            layout->setContentsMargins(24, 20, 24, 16);
            auto label = new QLabel(QObject::tr("已是最新版本"), dlg);
            label->setAlignment(Qt::AlignCenter);
            auto ok = new QPushButton(QObject::tr("确定"), dlg);
