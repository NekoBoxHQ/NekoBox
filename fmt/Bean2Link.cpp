...
namespace NekoGui_fmt {
    ...
    QString QUICBean::ToShareLink() {
        ...
        if (proxy_type == proxy_TUIC) {
            ...
            url.setPort(serverPort);
            QUrlQuery q;
            if (!password.isEmpty()) q.addQueryItem("auth", password);
            if (uploadMbps > 0) q.addQueryItem("up", Int2String(uploadMbps));
            if (downloadMbps > 0) q.addQueryItem("down", Int2String(downloadMbps));
            if (!obfs.isEmpty()) q.addQueryItem("obfs", obfs);
    ...
    QString WireGuardBean::ToShareLink() {
        QUrl url;
        url.setScheme("wireguard");
        url.setHost(serverAddress);
        url.setPort(serverPort);
        QUrlQuery q;
        if (!privateKey.isEmpty()) q.addQueryItem("private_key", privateKey);
        if (!publicKey.isEmpty()) q.addQueryItem("public_key", publicKey);
        if (!presharedKey.isEmpty()) q.addQueryItem("preshared_key", presharedKey);
        if (mtu != 1420) q.addQueryItem("mtu", Int2String(mtu));
    ...
    QString AnyTLSBean::ToShareLink() {
        ...
        QUrlQuery q;
        if (!sni.isEmpty()) q.addQueryItem("sni", sni);
        if (allowInsecure) q.addQueryItem("allowInsecure", "1");
        if (!q.isEmpty()) url.setQuery(q);
        if (!name.isEmpty()) url.setFragment(name);
        return url.toString(QUrl::FullyEncoded);
    }

    QString BrookBean::ToShareLink() {
        QUrl url;
        url.setScheme("brook");
        url.setUserName(password);
        url.setHost(serverAddress);
        url.setPort(serverPort);
        QUrlQuery q;
        if (!protocol.isEmpty() && protocol != "ws") q.addQueryItem("protocol", protocol);
        if (!q.isEmpty()) url.setQuery(q);
        if (!name.isEmpty()) url.setFragment(name);
        return url.toString(QUrl::FullyEncoded);
    }
