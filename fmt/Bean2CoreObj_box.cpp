...
namespace NekoGui_fmt {
    ...
    CoreObjOutboundBuildResult WireGuardBean::BuildCoreObjSingBox() {
        ...
        if (!presharedKey.isEmpty()) outbound["preshared_key"] = presharedKey;
        if (!reserved.isEmpty()) {
            QJsonArray ra;
            for (const auto &v: reserved.split(",")) {
                auto n = v.trimmed().toInt();
                if (n >= 0 && n <= 255) ra += n;
            }
            if (!ra.isEmpty()) outbound["reserved"] = ra;
        }
        result.outbound = outbound;
        return result;
    }
    ...
    CoreObjOutboundBuildResult AnyTLSBean::BuildCoreObjSingBox() {
        ...
        if (!sni.trimmed().isEmpty() || allowInsecure || NekoGui::dataStore->skip_cert) {
            QJsonObject tls{{"enabled", true}};
    ...
    CoreObjOutboundBuildResult BrookBean::BuildCoreObjSingBox() {
        CoreObjOutboundBuildResult result;
