...
namespace NekoGui_sub {
    ...
    void RawUpdater::update(const QString &str) {
        ...
        // Hysteria
        if (str.startsWith("hy://") || str.startsWith("hysteria://")) {
            needFix = false;
        ...
        // WireGuard
        if (str.startsWith("wireguard://") || str.startsWith("wg://")) {
            needFix = false;
        ...
        // Brook
        if (str.startsWith("brook://")) {
            needFix = false;
            ent = NekoGui::ProfileManager::NewProxyEntity("brook");
            auto ok = ent->BrookBean()->TryParseLink(str);
            if (!ok) return;
        }
